<template>
    <div class="fixed inset-0 z-50 flex items-center justify-center px-4">
        ...
        <!-- 弹窗主体 -->
        <div class="relative bg-white rounded-2xl shadow-2xl p-6 sm:p-8 max-w-sm sm:max-w-md mx-auto transform transition-all duration-300 ease-out"
            ...
            <div class="text-center">
                ...
                <h3 class="text-xl sm:text-2xl font-bold text-gray-900 mb-3 sm:mb-4 flex items-center justify-center">
                    <!-- 微信图标 -->
                    <svg class="w-6 h-6 sm:w-7 sm:h-7 mr-2 text-green-500" fill="currentColor" viewBox="0 0 24 24">
                        <path
                            d="M8.691 2.188C3.891 2.188 0 5.476 0 9.53c0 2.212 1.17 4.203 3.002 5.55a.59.59 0 0 1 .213.665l-.39 1.298c-.1.333.23.63.54.48l1.693-.82a.56.56 0 0 1 .486-.065c.711.248 1.46.379 2.147.379 4.8 0 8.691-3.288 8.691-7.342 0-4.054-3.891-7.342-8.691-7.342zm-2.46 9.638a.896.896 0 0 1-.896-.896.896.896 0 0 1 .896-.896.896.896 0 0 1 .896.896.896.896 0 0 1-.896.896zm4.92 0a.896.896 0 0 1-.896-.896.896.896 0 0 1 .896-.896.896.896 0 0 1 .896.896.896.896 0 0 1-.896.896z" />
                        <path
                ...
                <div class="mb-4 sm:mb-6">
                    <div class="inline-block p-3 sm:p-4 bg-gray-100 rounded-xl">
                        ...
                        <div v-if="loading"
                            ...
                            <div class="text-center">
                                <div
                                    class="animate-spin w-6 h-6 sm:w-8 sm:h-8 border-4 border-blue-500 border-t-transparent rounded-full mx-auto mb-2">
                                </div>
                        ...
                        <div v-else-if="imageUrl"
                            class="w-40 h-40 sm:w-48 sm:h-48 rounded-lg overflow-hidden border-2 border-blue-300">
                        ...
                        <div v-else
                            class="w-40 h-40 sm:w-48 sm:h-48 bg-gradient-to-br from-blue-100 to-blue-200 rounded-lg flex items-center justify-center border-2 border-dashed border-blue-300">
                            <div class="text-center">
                                <svg class="w-10 h-10 sm:w-12 sm:h-12 text-blue-500 mx-auto mb-2" fill="none"
...
const fetchMemberImageUrl = async () => {
    try {
        ...
        if (response.statusCode === 200) {
            ...
            console.warn('HomeModal: 响应状态码异常:', response)
            imageUrl.value = null
        }
    } catch (error) {
        console.error('HomeModal: 获取二维码图片URL失败:', error)
        imageUrl.value = null
    } finally {
        loading.value = false
    }
