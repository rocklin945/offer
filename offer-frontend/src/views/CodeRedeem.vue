<template>
    <div>
        <teleport to="body">
            <div class="fixed inset-0 bg-gradient-to-br from-teal-50 to-cyan-100">
...
const handleRedeem = async () => {
    ...
    try {
        // 调用卡密兑换API，传递账号、密码和卡密
        const res = await codeApi.redeemCode({
            account: form.value.userAccount,
            password: form.value.userPassword,
            code: form.value.code
        })

        if (res.statusCode === 200) {
            message.value = '卡密兑换成功！'
            success.value = true
